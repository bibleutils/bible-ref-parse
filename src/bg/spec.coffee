...
describe "Miscellaneous tests", ->
	...
	it "should handle verses (bg)", ->
		...
		expect(p.parse("Phlm СТ 6").osis()).toEqual("Phlm.1.6", "parsing: 'Phlm СТ 6'")
	...
	it "should handle 'ff' (bg)", ->
		...
		expect(p.parse("REV 3 И СЛ, 4:2 И СЛ").osis()).toEqual("Rev.3-Rev.22,Rev.4.2-Rev.4.11", "parsing: 'REV 3 И СЛ, 4:2 И СЛ'")
	...
	it "should handle book ranges (bg)", ->
		...
		expect(p.parse("Първа - Трета  Йоан").osis()).toEqual("1John.1-3John.1", "parsing: 'Първа - Трета  Йоан'")
