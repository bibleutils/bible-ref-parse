...
function makeGrammar() {
	...
	fs.writeFileSync(`${dir}/${lang}/grammar.pegjs`, out, { encoding: 'utf-8' });
	fs.writeFileSync(`${dir}/${lang}/grammar.pegjs.bak`, out, { encoding: 'utf-8' });
...
function makeRegexps() {
	...
	fs.writeFileSync(`${dir}/${lang}/regexps.coffee`, out, { encoding: 'utf-8' });
...
function formatVar(type: string, varName: string): string {
	const values: Array<string> = vars[varName]; // Assuming `vars` is an existing object
	...
	for (let index = 0; index < values.length; index++) {
		...
		values[index] = values[index].replace(/\[([^\]]*?[\u0080-\u{10FFFF}][^\]]*?)\]/ug, (match, group1) => `[${group1}]i`)
		.replace(/!(space ".+)/g, (match, group1) => `!(${group1})`)
		.trim();
...
function expandAbbrevVars(abbrev: string): string[] {
	...
	for (const value of vars[variable]) {
		...
		if (/^\$(?:FIRST|SECOND|THIRD|FOURTH|FIFTH)$/.test(variable) && (/^\d|^[IV]+$/.test(val))) {
			...
			const temp2 = abbrev.replace(new RegExp(safeVar + '([^.]|$)'), (match, group1) => `${val}.${group1}`);
